...
def generate_rsa_keys():
    """Gera chave RSA e exporta em PEM (privada e pública)"""
...
def encrypt_with_vdf_key(key_bytes: bytes, plaintext_bytes: bytes) -> str:
    """Usa VDF key (hash) para encriptar PEM com Fernet"""
    if len(key_bytes) < 32:
        raise ValueError("key_bytes deve ter pelo menos 32 bytes.")
...
def decrypt_with_vdf_key(key_bytes: bytes, ciphertext: str) -> str:
    ...
    try:
        ...
        logger.error(f"Erro ao desencriptar com VDF key", exc_info=True)
        raise
